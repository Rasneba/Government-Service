...
export default function DashboardPage() {
  ...
  if (loading) return <Layout><div className="flex items-center justify-center h-64 text-gray-500">Loading dashboard...</div></Layout>
  if (!data) return <Layout><div className="flex items-center justify-center h-64 text-red-500">Failed to load dashboard</div></Layout>

  const stats = [
    { label: 'Total Letters', value: data.totalLetters, icon: Mail, color: 'bg-blue-500' },
    ...
    { label: 'Pending', value: data.pendingLetters, icon: Clock, color: 'bg-yellow-500' },
    { label: 'Overdue', value: data.overdueLetters, icon: AlertTriangle, color: 'bg-red-500' },
  ]
    ...
    <Layout>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data.departmentStats}>
              ...
              <Bar dataKey="totalLetters" fill="#3b82f6" name="Total" /><Bar dataKey="completedLetters" fill="#10b981" name="Completed" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          {data.recentlyReceived.length === 0 ? <p className="text-gray-500 text-sm">No recent letters</p> : (
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          {data.recentActivities.length === 0 ? <p className="text-gray-500 text-sm">No recent activity</p> : (
