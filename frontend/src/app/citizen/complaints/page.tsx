  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">{t('citizen.complaints')}</h1>
        <button onClick={() => setShowForm(!showForm)} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700">{showForm ? t('common.cancel') : t('citizen.submitComplaint')}</button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-6 mb-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium mb-1">{t('applications.subject')} *</label><input type="text" value={subject} onChange={e => setSubject(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" required /></div>
            <div className="grid grid-cols-2 gap-2">
              ...
              <div><label className="block text-sm font-medium mb-1">{t('applications.priority')}</label><select value={priority} onChange={e => setPriority(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm">
                <option>Low</option><option>Normal</option><option>High</option><option>Urgent</option>
              </select></div>
            </div>
          </div>
          <div><label className="block text-sm font-medium mb-1">{t('applications.description')} *</label><textarea value={description} onChange={e => setDescription(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" rows={4} required /></div>
          <button type="submit" disabled={submitting} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 disabled:opacity-50">{submitting ? t('applications.creating') : t('citizen.submitComplaint')}</button>
        </form>
      )}

      {loading ? <div className="text-center py-12 text-gray-500">{t('common.loading')}</div> : complaints.length === 0 ? (
            ...
            <div key={c.id} className="bg-white border rounded-lg">
              ...
              {expandedId === c.id && (
                <div className="border-t p-4 space-y-4">
                  ...
                  {c.resolution && <div className="bg-green-50 p-3 rounded text-sm"><span className="font-medium">{t('common.details')}:</span> {c.resolution}</div>}
                  {c.comments && c.comments.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-sm font-medium">{t('applications.notes')}</div>
                      {c.comments.map(cm => (
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={commentText} onChange={e => setCommentText(e.target.value)} placeholder={t('applications.addNote')} className="flex-1 border rounded-lg px-3 py-2 text-sm" />
                    <button onClick={() => handleComment(c.id)} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">{t('common.submit')}</button>
                  </div>
                </div>
              )}
            </div>
