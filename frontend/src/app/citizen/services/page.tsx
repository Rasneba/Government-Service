...
export default function CitizenServicesPage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-6">{t('services.categories')}</p>

      {reissueServices.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
          <h2 className="font-semibold text-amber-800 mb-2">{t('citizen.reissueTitle')}</h2>
          <p className="text-sm text-amber-700 mb-3">{t('citizen.reissueSubtitle')}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {reissueServices.map(st => (
              <button key={st.id} onClick={() => router.push(`/citizen/applications/new?serviceTypeId=${st.id}`)} className="bg-white border border-amber-300 rounded-lg p-3 text-left hover:bg-amber-100 transition">
                ...
                <div className="text-xs text-gray-500 mt-1">{t('services.fee')}: {st.fee > 0 ? `ETB ${st.fee}` : 'Free'} | {st.estimatedDays || 'Varies'} days</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button onClick={() => setSelectedCategory(null)} className={`px-4 py-2 rounded-full text-sm ${!selectedCategory ? 'bg-green-600 text-white' : 'bg-white border text-gray-600 hover:bg-gray-50'}`}>{t('common.all')} ({serviceTypes.length})</button>
        {categories.map(c => (
              ...
              <div key={st.id} className={`bg-white border rounded-lg p-6 hover:shadow-md transition-shadow ${isReissue ? 'border-amber-200' : ''}`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    {isReissue && <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded mt-1 inline-block">{t('citizen.reissueTitle')}</span>}
                  </div>
                  <span className="text-xs bg-gray-100 px-2 py-1 rounded">{st.code}</span>
                </div>
                <p className="text-sm text-gray-600 mb-4">{st.description || t('common.noData')}</p>
                <div className="text-sm text-gray-500 space-y-1 mb-4">
                  <div className="flex justify-between"><span>{t('services.categories')}:</span><span>{st.categoryName || '-'}</span></div>
                  <div className="flex justify-between"><span>{t('services.estimatedDays')}:</span><span>{st.estimatedDays ? `${st.estimatedDays} days` : 'Varies'}</span></div>
                  <div className="flex justify-between"><span>{t('services.fee')}:</span><span className="font-medium">{st.fee > 0 ? `ETB ${st.fee.toLocaleString()}` : 'Free'}</span></div>
                  {st.requiresPoliceVerification && (
                    <div className="flex items-center gap-1 text-slate-600 text-xs"><Shield size={12} /> {t('services.policeVerification')}</div>
                  )}
                </div>
                <button onClick={() => router.push(`/citizen/applications/new?serviceTypeId=${st.id}`)} className={`w-full py-2 rounded-lg text-sm ${isReissue ? 'bg-amber-600 text-white hover:bg-amber-700' : 'bg-green-600 text-white hover:bg-green-700'}`}>
                  {isReissue ? t('citizen.submitRequest') : t('services.applyNow')}
                </button>
              </div>
            )
          })}
